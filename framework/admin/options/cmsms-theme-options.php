...
if (CMSMS_TIMETABLE) {
	require_once(CMSMS_OPTIONS . '/cmsms-theme-options-tt-event.php');
}
...
function cmsms_admin_enqueue_scripts($hook) {
	if ( 
		...
		wp_localize_script('cmsms_theme_options_js', 'cmsms_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'yoga-fit'), 
			'select_format' => 		esc_attr__('Please select the format.', 'yoga-fit'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'yoga-fit'), 
			'want_remove' => 		esc_html__('Do you realy want to remove this item?', 'yoga-fit'), 
...
function show_cmsms_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'yoga-fit') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'yoga-fit') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'yoga-fit') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'yoga-fit') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'yoga-fit') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'yoga-fit') . '" />' . 
					'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'yoga-fit') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'yoga-fit') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'yoga-fit') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . __('Blank Field', 'yoga-fit') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'yoga-fit') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . __('Posts', 'yoga-fit') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Pages', 'yoga-fit') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Projects', 'yoga-fit') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'yoga-fit') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'yoga-fit') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'yoga-fit') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsms_gallery_item">' . 
							'<img src="' . esc_url($id[1]) . '" alt="" data-id="' . $id[0] . '" class="cmsms_gallery_image" />' . 
							'<a href="#" class="cmsms_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'yoga-fit') . '"></a>' . 
						'</li>';
					}
				}
			}
