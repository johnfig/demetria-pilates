...
function cmsms_settings_page_header() {
	...
	echo '<div id="icon-options-general" class="icon32">' . 
		'<br />' . 
	'</div>' . 
	'<h2 style="padding-top:12px;">' . esc_html($settings_output['cmsms_page_title']) . '</h2>';
	...
	if ($tabs != '') {
		...
		foreach ($tabs as $tab => $name) {
			...
			$links[] = '<a class="' . esc_attr($class) . '" href="?page=' . esc_attr($page) . '&tab=' . esc_attr($tab) . '"' . (($tab == 'recaptcha') ? ' style="background-color:#fdffc6;' . (($class == 'nav-tab nav-tab-active') ? ' border-bottom-color:#fdffc6;' : '') . '"' : '') . '>' . esc_html($name) . '</a>';
		}
