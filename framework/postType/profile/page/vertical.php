...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	if (has_post_thumbnail()) {
		...
		cmsms_thumb_rollover(get_the_ID(), 'square-thumb', false, true, false, false, false, false, false, true, true);
		
		echo '</div>' . "\n";
	} else {
		echo '<div class="pl_img"><a href="' . esc_url(get_permalink()) . '"><span class="pl_noimg cmsms_theme_icon_status_post"></span></a></div>' . "\n";
	}
		...
		echo '<div class="pl_social">' . "\n";
			...
			if ($cmsms_profile_social != '') {
				...
				foreach ($cmsms_profile_social as $social_icons) {
					...
					echo '<li>' . "\n\t" . 
						'<a href="' . esc_url($social_icon[1]) . '" class="' . esc_attr($social_icon[0]) . '" title="' . esc_attr($social_icon[2]) . '"' . (($social_icon[3] == 'true') ? ' target="_blank"' : '') . '></a>' . "\r" . 
					'</li>' . "\n";
				}
