...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php 
		cmsms_thumb_rollover(get_the_ID(), 'project-thumb', false, true, true, false, $cmsms_project_video_type, $cmsms_project_video_link, $cmsms_project_video_links, false, true, $cmsms_project_link_redirect, $cmsms_project_link_url, $more);
		
		
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="slider_project_inner">';
				
				($title) ? cmsms_slider_post_heading(get_the_ID(), 'project', 'h4', true, $cmsms_project_link_redirect, $cmsms_project_link_url) : '';
