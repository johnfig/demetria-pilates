...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsms_' . $cmsms_project_columns); ?>>
	...
	if ($cmsms_project_title == 'true') {
		...
		'<div class="cmsms_project_header_inner">';
			cmsms_project_title_nolink(get_the_ID(), 'h2');
		echo '</div>' . 
		'</header>';
	}
		...
		if (!post_password_required()) {
			if (sizeof($cmsms_project_images) > 0 && $cmsms_project_images[0] != '') {
				...
				foreach ($cmsms_project_images as $cmsms_project_image) {
					...
					echo '<div class="' . esc_attr($cmsms_project_img_columns) . '">' . 
						'<figure class="cmsms_img_rollover_wrap preloader">' . 
							...
							'<div class="cmsms_img_rollover">' . 
								'<a href="' . esc_url($link_href[0]) . '" rel="ilightbox[' . get_the_ID() . '_' . $unique_id . ']" title="' . cmsms_title(get_the_ID(), false) . '" class="cmsms_image_link no_open_link"><span class="cmsms_theme_icon_search"></span></a>' . 
							'</div>' . 
						'</figure>' . 
					'</div>';
	...
	if ($project_sidebar == 'true') {
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				echo '<div class="project_details entry-meta">';
					...
					if ($cmsms_project_details_title != '') {
						echo '<h4 class="project_details_title">' . $cmsms_project_details_title . '</h4>';
					}
					...
					cmsms_project_features('details', $cmsms_project_features, false, 'h4', true);
			...
			if ($cmsms_project_sharing_box == 'true') {
				cmsms_sharing_box(esc_html__('Like this project?', 'yoga-fit'), 'h4');
			}
