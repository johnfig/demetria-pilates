...
<article id="post-<?php the_ID(); ?>" <?php post_class($cmsms_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	<div class="project_outer">
	<?php 
		cmsms_thumb_rollover(get_the_ID(), $project_img_size, $rollover, false, false, false, $cmsms_project_video_type, $cmsms_project_video_link, $cmsms_project_video_links, false, true, $cmsms_project_link_redirect, $cmsms_project_link_url);
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">' . 
				'<div class="project_inner_ovh">' . 
					'<div class="project_in_inner">';
						
						echo '<div class="cmsms_project_cont_wrap">';
