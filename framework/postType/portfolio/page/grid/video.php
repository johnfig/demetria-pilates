...
<article id="post-<?php the_ID(); ?>" <?php post_class(); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		cmsms_thumb_rollover(get_the_ID(), $project_thumb_size, false, true, true, false, $cmsms_project_video_type, $cmsms_project_video_link, $cmsms_project_video_links, $project_thumb_high, true, $cmsms_project_link_redirect, $cmsms_project_link_url, $rollover);
		
		if (!$title) {
			echo '<div class="dn">';
				cmsms_project_heading(get_the_ID(), 'h4');
			echo '</div>';
		}
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				($title) ? cmsms_project_heading(get_the_ID(), 'h4', true, $cmsms_project_link_redirect, $cmsms_project_link_url) : '';
