		...
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_profile_post_cat']) {
				$out = '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'yoga-fit') . '</div>' . 
...
function cmsms_profile_comments($template_type = 'page', $show = true) {
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = '<a class="cmsms_profile_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'yoga-fit') . ' ' . esc_attr(get_the_title()) . '">' . esc_html(get_comments_number()) . '</a>';
		} elseif ($template_type == 'post') {
			$cmsms_option = cmsms_get_global_options();
			$out = '';
...
function cmsms_profile_more($cmsms_id, $show = true) {
	$cmsms_profile_read_more = get_post_meta($cmsms_id, 'cmsms_profile_read_more', true);
	
	
	if ($cmsms_profile_read_more == '') {
		$cmsms_profile_read_more = esc_attr__('Profile', 'yoga-fit');
	}
	
	
	$out = '<a class="button cmsms_profile_read_more" href="' . esc_url(get_permalink($cmsms_id)) . '">' . esc_html($cmsms_profile_read_more) . '</a>';
